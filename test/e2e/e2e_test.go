...
// TestClusterResourceOverrideCPURequestToRequestPercentTakesPrecedence verifies
// the field precedence ordering when both cpuRequestToLimitPercent and
// cpuRequestToRequestPercent are configured together on a ClusterResourceOverride.
// cpuRequestToRequestPercent always runs last and derives from the pod's original
// CPU request (preserved via annotation), overwriting the intermediate value
// already written by cpuRequestToLimitPercent.
func TestClusterResourceOverrideCPURequestToRequestPercentTakesPrecedence(t *testing.T) {
	client := helper.NewClient(t, options.config)
...
// TestResourceOverrideConflictEmitsWarningEventOnLoser verifies that when two
// ResourceOverride objects in the same namespace both match a pod (empty podSelector),
// the lexicographically-first name wins and the losing object emits a
// Warning/OverrideConflict event rather than blocking admission.
func TestResourceOverrideConflictEmitsWarningEventOnLoser(t *testing.T) {
	client := helper.NewClient(t, options.config)
	...
	const winnerName = "a-ro-conflict"
	const loserName = "b-ro-conflict"
